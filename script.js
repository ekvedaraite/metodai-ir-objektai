...
// 1.
const male = data.filter(person => person.gender.toLowerCase() == 'male').length
console.log("Vyrų skaičius:", male)

// 2.
const newerCars = data.filter(person => person.car_year > 2000).map(person => person.id)
console.log("Žmonės, kurių automobiliai naujesni, nei 2000 metų: ", newerCars)

// 3.
const sortedNames = data.filter(person => person.shirt_size == "XS" || person.shirt_size == "S").map(person => person.first_name).sort()
console.log("Žmonės, kurių dydžiai XS arba S pagal abėcėlę:", sortedNames)

// 4.
const sortedInfo = data.filter(person => person.shirt_size == "XS" || person.shirt_size == "S").map(person => ({ id: person.id, first_name: person.first_name, shirt_size: person.shirt_size })).sort((a, b) => a.first_name.localeCompare(b.first_name))
console.log("ID, vardai ir marškinių dydžiai pagal abėcėlę:", sortedInfo)
